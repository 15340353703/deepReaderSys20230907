...
public class DiscussionServiceImpl implements DicussionService {
    ...
    public List<Discussion> getDiscussionList(Integer page,Integer pageSize) {
        ...
        for (Discussion discussion:discussionList){
            ...
            //写入数据
            discussion.setUserName(user.getYourname());
    ...
    @Override
    public List<Discussion> getUserDiscussionList(User user, Integer page, Integer pageSize) {
        //获取分页插件对象
        PageHelper pageHelper=new PageHelper();
        //开始分页，指定分页参数
        PageMethod.startPage(page,pageSize);
        List<Discussion> discussionList = discussionMapper.getUserDiscussionList(user.getId());

        //获取创建人姓名和作业信息
        for (Discussion discussion:discussionList){
            User createUser=userMapper.findUserById(discussion.getUserId());
            Classify classify=classifyMapper.findClassifyById(discussion.getClassifyId());

            //写入数据
            discussion.setUserName(createUser.getYourname());
            discussion.setClassifyName(classify.getName()+"-"+classify.getWork());
        }
        return discussionList;
    }

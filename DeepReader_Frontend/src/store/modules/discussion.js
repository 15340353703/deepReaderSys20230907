import { getDiscussionList,getDiscussionById,submitDiscussion,getUserDiscussionList } from '@/api/discussion'
...
export default {
    ...
    actions: {
        getDiscussionList({ commit },  page, pageSize ) {
          ...
          getUserDiscussionList({ commit,state },  page, pageSize ) {
            return new Promise((resolve, reject) => {
              getUserDiscussionList(state.token,page, pageSize).then((response) => {
                const { data } = response
                if (!data) {
                  reject('请求数据失败.')
                }

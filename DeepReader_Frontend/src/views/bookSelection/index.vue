              ...
              <div style="padding: 14px">
                ...
                <div class="bottom clearfix">
                  ...
                  size="mini" round
                    v-if="!item.have_read"
                    type="warning"
                    @click="startRead(item.id)"
                  >开始阅读</el-button>&ensp;
                  <el-button
                    size="mini" round
                    v-if="!item.have_read"
                    type="warning"
                    @click="startkwlRead(item.id)"
                  >开始kwl阅读</el-button>&ensp;
                  <el-button
                  size="mini" round
                    v-if="item.have_read"
                    type="success"
...
export default {
  ...
  methods: {
    startkwlRead(id){
      const data = {userid: getToken(),behavior:'开始kwl阅读',bookid:id,stamptime:new Date()}
      service.post('/user/readlog',data).then(res=>{
        console.log('记录开始kwl阅读')
      })
      this.$router.push({path: '/study/index', query: {bookid: id}})
    },
    startRead(id){
      const data = {userid: getToken(),behavior:'开始阅读',bookid:id,stamptime:new Date()}
      service.post('/user/readlog',data).then(res=>{
        console.log('记录开始阅读')
      })
      this.$router.push({path: '/study/components/index', query: {bookid: id }})
    },
    createQuestion(id){
      const data = {userid: getToken(),behavior:'创建问题',bookid:id,stamptime:new Date()}
      service.post('/user/readlog',data).then(res=>{
        console.log('记录创建问题')
      })
      this.$router.push({path: '/claimQuestion/index', query: {bookid: id}})
    },
    ...
    getAllBookList(page, pageSize) {
      ...
      this.getUserBookList({ page, pageSize }).then((response) => {
        // console.log("booklist2:"+response.list)

    ...
    <el-main style="text-align: center">
      ...
      <div>
        <el-col v-for="(item, id) in this.list" :key="id" :span="6">
          <el-col :span="20">
            <el-card v-if="item.have_read" :body-style="{ padding: '0px' }" shadow="hover">
              <img
              ...
              <div style="padding: 14px">
                ...
                <div class="bottom clearfix">
                  ...
                  <el-button
                    ...
                    round
                    type="success"
                    @click="$router.push('./components/index?bookId='+item.id)">查看批注反馈</el-button>
<!--                    @click="$router.push('./components/index?bookId='+item.id)">-->
<!--                  查看批注反馈</el-button>-->
                </div>
              </div>
            </el-card>
...
export default {
  ...
  methods: {
    // startAnnotation(id) {
    //   // const data = { userid: getToken(), behavior: '开始查看批注反馈', bookid: id, stamptime: new Date() }
    //   // service.post('/user/readlog', data).then(res => {
    //   //   console.log('记录开始深度提问')
    //   // })
    // @click="$router.push('/components/index?bookId='+item.id)"
    //   this.$router.push({ path: '/components/index', query: { bookid: id }})
    // },
